...
router.post('/paypal/capture', auth, async (req, res) => {
  try {
    ...
    const result = await applyPurchase(user, { kind, bundleId, tier, method: 'paypal', paypalOrderId: orderId, last4: '' });
    await user.save();
    res.json({ ...result, sandbox: cap.sandbox });
  } catch (err) {
...
// body: { kind, bundleId | tier, card: { number, expMonth, expYear, cvc, name? } }
router.post('/credit-card', auth, async (req, res) => {
  try {
    ...
    const result = await applyPurchase(user, { kind, bundleId, tier, method: 'credit-card', paypalOrderId: '', last4: v.last4 });
    await user.save();
...
async function applyPurchase(user, { kind, bundleId, tier, method, paypalOrderId, last4 }) {
  if (kind === 'tokens') {
    const bundle = TOKEN_BUNDLES.find(b => b.bundleId === bundleId);
    ...
    user.payments.push({
      ...
      tokensAwarded: bundle.tokens, method, paypalOrderId, cardLast4: last4 || '',
    });
  ...
  if (kind === 'subscription') {
    ...
    // Sub also drops a starter weekly bonus right away.
    user.tokens = (user.tokens || 0) + plan.weeklyTokens;
    user.payments.push({
      kind: 'subscription', bundleId: plan.tier, amountUSD: plan.priceUSD,
      tokensAwarded: plan.weeklyTokens, method, paypalOrderId, cardLast4: last4 || '',
    });
    const rewards = awardRewards(user);
    return {
